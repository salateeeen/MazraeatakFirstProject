...
const getFarmsLocations = handleAsyncError(async (req, res) => {
  const {distance, latlng, unit, ...filters} = req.query;
 
  const locations = await FarmsLocations(filters, distance, latlng, unit, req.user);
...
const getFarmsWithinDistance = handleAsyncError(async (req, res) => {
  const {distance, latlng, unit} = req.params;
  
  const farms = await getFarmsWithinDistanceService(req.query, distance, latlng, unit);
