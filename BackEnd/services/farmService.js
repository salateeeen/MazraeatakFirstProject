...
const FarmsLocations = async (queryParams, distance, latlng, unit, user = null) => {
...
const getFarmsWithinDistanceService = async (queryParams, distance, latlng, unit) => {
  let [lat, lng] = [];
  if (distance) {
    if (!lat || !lng) {
      throw new AppError("Please provide a latitude and longitude.", 400);
  ...
  const Query = new QueryClass(Farm.find(query), queryParams);
